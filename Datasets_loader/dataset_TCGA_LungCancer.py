...
class TCGA_LungCancer(torch.utils.data.Dataset):
    ...
    def __init__(self, train=True, transform=None, downsample=0.2, preload=False, patch_downsample=1.0, scale=1):
        ...
        dir_LUAD = "/home/xiaoyuan/Data3/TCGA_LungCancer/processed_LUAD/pyramid/xiaoyuan/"
        dir_LUSC = "/home/xiaoyuan/Data3/TCGA_LungCancer/processed_LUSC/pyramid/xiaoyuan/"
        file_test_id = "/home/xiaoyuan/Data3/TCGA_LungCancer/TEST_ID.csv"
...
class TCGA_LungCancer_Feat(torch.utils.data.Dataset):
    ...
    def __init__(self, train=True, downsample=1.0, return_bag=False):
        ...
        bags_csv = '/home/xiaoyuan/Data/tcga-dataset/TCGA.csv'
        ...
        self.patch_corresponding_slide_label = np.concatenate(self.patch_corresponding_slide_label).astype(np.longlong)
        self.patch_corresponding_slide_index =np.concatenate(self.patch_corresponding_slide_index).astype(np.longlong)
        ...
        self.patch_label_all = np.zeros([self.patch_feat_all.shape[0]], dtype=np.longlong)  # Patch label is not available and set to 0 !
        # 3.do some statistics
...
class TCGA_LungCancer_CLIPFeat(torch.utils.data.Dataset):
    def __init__(self, feat_dir="./output_TCGA_feat_224x224_scale1_CLIP(RN50)", train=True, return_bag=True):
    # def __init__(self, feat_dir="./output_TCGA_feat_224x224_scale1_CLIP(ViTB32)", train=True, return_bag=True):
        ...
        self.patch_label = np.zeros_like(self.patch_corresponding_slide_label)  # dummy
        print("Feat Loaded")
        ...
        for i in tqdm(available_slide_index, desc='Sort by slide'):
            ...
            self.slide_patch_label_all.append(np.zeros(idx_from_same_slide.shape[0]).astype(np.longlong))
        print("Feat Sorted")

    def __getitem__(self, index):
        if self.return_bag:
            return self.slide_feat_all[index], [self.slide_patch_label_all[index], self.slide_label_all[index], index], index
        else:
...
def get_bag_feats(csv_file_df):
    feats_csv_path = '/home/xiaoyuan/Data/tcga-dataset/tcga_lung_data_feats/' + csv_file_df.iloc[0].split('/')[1] + '.csv'
